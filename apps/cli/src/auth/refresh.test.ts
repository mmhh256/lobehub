...
describe('describeTokenLookup', () => {
  it('says the stored login survived when the server never answered', () => {
    const message = describeTokenLookup({ detail: 'fetch failed', status: 'unavailable' });

    expect(message).toContain('fetch failed');
    expect(message).toContain('untouched');
    // The whole point: it must not send the user off to re-authenticate.
    expect(message).not.toMatch(/run .*login/i);
  });

  it('names the reason a stored login was spent', () => {
    expect(describeTokenLookup({ detail: 'invalid_grant', status: 'spent' })).toContain(
      'invalid_grant',
    );
  });

  it('does not send the user to sign in again over a refusal the credential did not cause', () => {
    const message = describeTokenLookup({ detail: 'HTTP 403', status: 'refused' });

    expect(message).toContain('HTTP 403');
    expect(message).toContain('signing in again will not change the answer');
  });
