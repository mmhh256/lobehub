...
/**
 * The line to print when a command cannot run because of the token, or `undefined` when the
 * lookup carries no diagnosis of its own and the caller's own "log in first" message fits.
 */
export function describeTokenLookup(lookup: TokenLookup): string | undefined {
  switch (lookup.status) {
    case 'spent': {
      return `The stored login is no longer valid (${lookup.detail}).`;
    }
    case 'refused': {
      return `The server refused the refresh request: ${lookup.detail}. The stored login is intact and signing in again will not change the answer — check the server URL and anything sitting in front of it.`;
    }
    case 'unavailable': {
      return `Could not reach the server to refresh the access token: ${lookup.detail}. The stored login is untouched — this is the network or the server, not a signed-out session, so retry in a moment.`;
    }
    default: {
      return undefined;
    }
